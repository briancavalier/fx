...
| `intermediate/checkpointed-state.ts` | Plain state recovery compared with checkpointed state rollback for scoped catch regions. | Readers deciding when recovery should preserve or roll back partial state changes. | `node --import tsx examples/intermediate/checkpointed-state.ts` |
