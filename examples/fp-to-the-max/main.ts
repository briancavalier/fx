import { Effect, Env, Fail, Fx } from '../../src';
...
export const toInteger = (s: string) => {
  ...
  return Number.isInteger(i) ? Fx.ok(i) : Fail.fail(`"${s}" is not an integer`);
};
...
// to guess it.
const play = (name: string, max: number) =>
  Fx.fx(function* () {
    ...
    const result = yield* read(
      `Dear ${name}, please guess a number from 1 to ${max}: `
    );
...
const checkContinue = (name: string) =>
  Fx.fx(function* () {
    while (true) {
      ...
      switch (answer.trim().toLowerCase()) {
        ...
        case 'n':
          return false;
        default:
          yield* print('Please enter y or n.');
      }
    }
  });
