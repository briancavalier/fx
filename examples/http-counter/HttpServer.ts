...
import { Env, Fork, Fx, Log, Stream } from "../../src";
//----------------------------------------------------------------------
// Http Server example
...
export const httpServer = Fx.bracket(
  Fx.fx(function* () {
    const { port } = yield* Env.get<{ port: number }>();
    return createServer().listen(port);
  }),
  (server) => Fx.ok(void server.close()),
  (server) =>
    Stream.withEnqueue<Connection>((q) => {
      server.on("request", (request, response) =>
        q.enqueue({ request, response })
      );
      return q;
    })
);
...
export const runServer = <E>(
  handleRequest: (r: Request) => Fx.Fx<E, Response>
) =>
  Stream.forEach(httpServer, ({ request, response }) =>
    Fx.fx(function* () {
      yield* Fork.fork(
        Fx.fx(function* () {
          const r = yield* handleRequest({
            method: "GET",
            url: "",
            ...request,
          });
          yield* Log.info("Handled request", {
